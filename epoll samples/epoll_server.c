...
/*  LT工作模式特点：稳健但效率低 */
void lt_process(epoll_event* events, int number, int epoll_fd, int listen_fd)
{
...
/* ET工作模式特点：高效但潜在危险 */
void et_process(epoll_event* events, int number, int epoll_fd, int listen_fd)
{
    char buf[BUFFER_SIZE];
	int i;
	for(i = 0; i < number; i++)
	{
	    ...
	    if(sockfd == listen_fd)
		{
		    struct sockaddr_in client_address;
			socklen_t client_addrlength = sizeof(client_address);
			int connfd = accept(epoll_fd, connfd, true);  //使用et模式
...
int main(int argc, char*argv[])
{
	...
	const char* ip = agrv[1];
	int port = atoi(argv[2]);
	
	int ret = -1;
	struct sockaddr_in address;
	bzero(&address, sizeof(affress));
	address.sin_family = AF_INET;
	inet_pton(AF_INET, ip, &address.sin_addr);
	address.sin_port = htons(port);
	
	int listen_fd = socket(PF_INET, SOCK_STEARM, 0);
	if(listen_fd < 0)
	{
	...
	ret = bind(listen_fd, (struct sockadrr*)&address, sizeof(address));
	if(ret == -1)
	{
	...
	while(1)
	{
		int ret = epoll_wait(epoll_fd, events, MAX_EVENT_NUMBER, -1);  //-1表示永久等待
		if(ret < 0)
		{
		...
		if(ENABLE_ET)
		{
			et_process(events, ret, epoll_fd, lsiten_fd);
		}
		else
		{
			lt_process(events, ret, epoll_fd, lsiten_fd);  
		}
